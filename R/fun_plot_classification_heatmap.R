#### NOTE This is a modified version of the main heatmap function in LundTaxR - specifically designed for the application note manuscript /Adam
...
#' @param row_height Row height in mm for gene-labelled heatmap sections. Default is 6.
#' 
...
plot_classification_heatmap = function(these_predictions = NULL,
  ...
  hm_luminal_tfs = Heatmap(plot_data[genes_ud,,drop = FALSE],
                           ...
                           border_gp = gpar(lwd = 1),
                           show_heatmap_legend = FALSE,
                           height = unit(length(genes_ud) * row_height, "mm"),
                           row_title_rot = 0)
  ...
  hm_luminal_gen = Heatmap(plot_data[genes_upk,,drop = FALSE],
                           ...
                           border_gp = gpar(lwd = 1),
                           show_heatmap_legend = FALSE,
                           height = unit(length(genes_upk) * row_height, "mm"),
                           row_title_rot = 0)
  ...
  if(length(genes_circ) == 5){
    ...
    hm_circuitscore = Heatmap(plot_data[genes_circ,, drop = FALSE],
                              ...
                              border_gp = gpar(lwd = 1),
                              show_heatmap_legend = FALSE,
                              height = unit(length(genes_circ) * row_height, "mm"),
                              row_title_rot = 0)
  ...
  if(length(genes_basq) == 4) {
    ...
    hm_basq = Heatmap(plot_data[genes_basq,, drop = FALSE],
                      ...
                      border_gp = gpar(lwd = 1),
                      show_heatmap_legend = FALSE,
                      height = unit(length(genes_basq) * row_height, "mm"),
                      row_title_rot = 0)
  ...
  if(length(genes_erbb) == 3){
    ...
    hm_erbscore = Heatmap(plot_data[genes_erbb,,drop = FALSE],
                          ...
                          border_gp = gpar(lwd = 1),
                          show_heatmap_legend = FALSE,
                          height = unit(length(genes_erbb) * row_height, "mm"),
                          row_title_rot = 0)
  ...
  hm_adhesion = Heatmap(plot_data[genes_ad,,drop = FALSE],
                        ...
                        border_gp = gpar(lwd = 1),
                        show_heatmap_legend = FALSE,
                        height = unit(length(genes_ad) * row_height, "mm"),
                        row_title_rot = 0)
  ...
  hm_myc = Heatmap(plot_data[genes_myc,,drop = FALSE],
                   ...
                   border_gp = gpar(lwd = 1),
                   show_heatmap_legend = FALSE,
                   height = unit(length(genes_myc) * row_height, "mm"),
                   row_title_rot = 0)
  ...
  hm_neuronal = Heatmap(plot_data[genes_ne,,drop = FALSE],
                        ...
                        height = unit(length(genes_ne) * row_height, "mm"),
                        row_title_rot = 0)
  ...
  if(save_to_file){
    #set PDF outputs
    if(out_format == "pdf"){
      cairo_pdf(paste0(out_path, plot_title, "_heatmap_scores.pdf"),
                width = plot_width,
                height = plot_height,
                family = "Arial")
      #set PNG outputs
  ...
  if(plot_signature_scores) {
    ...
    #color functions for molecular grade scores
    col_fun_grade_1999 =
      circlize::colorRamp2(c(0, 0.5, 1),
                           c("#2166AC", "white", "#B2182B"))
    ...
    colour_obj = list(`Predicted Subtype` = lund_colors$lund_colors,
                      ...
                      `Mol. grade score (WHO1999)` = col_fun_grade_1999,
                      `Mol. grade score (WHO2022)` = col_fun_grade_2022,
    ...
    hm_a_sign <- HeatmapAnnotation(`Proliferation Score` = these_predictions$scores$proliferation_score,
                                   `Mol. grade score (WHO1999)` = these_predictions$scores$molecular_grade_who_1999_score,
                                   `Mol. grade score (WHO2022)` = these_predictions$scores$molecular_grade_who_2022_score,
    ...
    if(length(available_immune_names) > 0) {
      hm_immune_scores_sig <- Heatmap(
        ...
        column_title = NULL,
        row_title = NULL,
        height = unit(length(available_immune_names) * row_height, "mm"),
    ...
    if(length(available_stromal_names) > 0) {
      hm_stroma_scores_sig <- Heatmap(
        ...
        column_title = NULL,
        row_title = NULL,
        height = unit(length(available_stromal_names) * row_height, "mm"),
